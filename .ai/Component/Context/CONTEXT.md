...
- **In new code, inject the context service** (`LanguageContext`, `ShopContext`, `CurrencyContext`, `EmployeeContext`) — never read values from the legacy `Context::getContext()` global, and never pass legacy-context values into services, query builders, or repositories. This is a recurring review point: e.g. a grid query builder needing the current language must depend on `LanguageContext`, not on the injected legacy context. `buildLegacyContext()` exists only to sync the old global during migration — it is not an entry point for reading
